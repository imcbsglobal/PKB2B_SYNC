...
def get_total_count(conn):
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM DBA.acc_productproduct WHERE TRIM(settings) LIKE '#EC%'")
...
def run_sync():
    ...
    try:
        print("[SYNC] Fetching acc_productproduct WHERE TRIM(settings) LIKE '#EC%'")
