...
def fetch_photos(conn):
    ...
    while True:
        ...
        if not rows:
            break
        yield [{"code": r[0], "url2": r[1]} for r in rows]
    cursor.close()
...
def push_to_api(cfg, records, is_first_batch=False):
    ...
    response = requests.post(url, json={"records": records, "is_first_batch": is_first_batch})
...
def run_sync():
    ...
    try:
        print("[SYNC] Fetching acc_productphoto joined with acc_product WHERE #EC%")
